   ...
   {  
      ...
      if(orderSelectResult == true)
      {
         ...
         if(OrderCloseTime() == 0)
         {
            ...
            CheckCloseConditions(MovingAverages,ticket);
         }
         //No sell order is in place
         else
         {
            //Execute buy order if conditions are met
            BeginSellOrder(MovingAverages, ticket);
         }
      }
      else
      {
...
void BeginSellOrder(double &MovingAverages[5], int &ticket)
{
  ...
  //Alert(Bid);
  if((FirstSpread > ND(Bid*.00142363)) && (SecondSpread > ND(Bid*.00056255)))
  {
      ticket = MarketOrderSend(Symbol(), OP_SELL, Lots, ND(Bid), 10*int(MyPoint/Point()),0,0, "Set by Ribbon Strategy V2", MagicBuy);
      if(ticket < 0)
      {
